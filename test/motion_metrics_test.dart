...
void main() {
  group('MotionMetrics', () {
    ...
    test('constant series yields maximum consistency', () {
      final samples = List.generate(30, (i) => _sample(i * 0.05, 0.4, 0.4));
      final metrics = MotionMetrics.compute(
        samples: samples,
        expectedAmplitude: 0.5,
      );
      expect(metrics.consistency, equals(100));
      expect(metrics.intensity, equals(0));
      expect(metrics.frequency.hertz, equals(0));
    });
    ...
    test('step function input stays bounded', () {
      final samples = <MotionSample>[];
      for (var i = 0; i < 40; i++) {
        final x = i < 20 ? 0.4 : 0.6;
        samples.add(_sample(i * 0.02, x, 0.5));
      }
      final metrics = MotionMetrics.compute(
        samples: samples,
      ...
      expect(metrics.consistency, inInclusiveRange(0, 100));
      expect(metrics.direction.stability, inInclusiveRange(0, 100));
      expect(metrics.intensity, inInclusiveRange(0, 100));
      expect(metrics.frequency.confidence, inInclusiveRange(0, 1));
    });

    test('alternating series stays finite and deterministic', () {
      final samples = List.generate(
        60,
        (i) => _sample(i * 0.01, i.isEven ? 0.45 : 0.55, 0.5),
      );
    ...
    test('scaled series remain in valid ranges', () {
      ...
      expect(scaledMetrics.intensity, inInclusiveRange(0, 100));
    });
    ...
    test('noisy deterministic series stays finite', () {
      ...
      for (var i = 0; i < 100; i++) {
        ...
        samples.add(_sample(i * 0.01, pos.x, pos.y));
      }
      final metrics = MotionMetrics.compute(
        samples: samples,
        expectedAmplitude: 0.4,
      ...
      expect(metrics.frequency.confidence.isNaN, isFalse);
      expect(metrics.consistency, inInclusiveRange(0, 100));
      expect(metrics.frequency.confidence, inInclusiveRange(0, 1));
    });
